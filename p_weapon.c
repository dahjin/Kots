...
void Blaster_Fire (edict_t *ent, vec3_t g_offset, int damage, qboolean hyper, int effect)
{
	...
	if (ent->character->cur_hyperblaster >= 10 && ent->character->rune && ent->character->rune->hyperblaster > 0)
		fire_blaster_ex (ent, start, forward, damage, 1800, effect | EF_COLOR_SHELL, RF_SHELL_BLUE, true, MOD_HYPERBLASTER); //level 10 + rune stuns
...
void Machinegun_Fire (edict_t *ent)
{
	...
	if (ent->character->cur_machinegun >= 4 && ent->client->ps.gunframe == 8)
	{
		...
		fire_blaster_ex(ent, start, forward, damage, 1800, EF_HYPERBLASTER, 0, false, MOD_MACHINEGUN);
	}
