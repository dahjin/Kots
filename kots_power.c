...
void Kots_CharacterAddExpack(edict_t *ent)
{
	if (Kots_CharacterCanAddPower(ent))
	{
		...
		else if (ent->character->level < 8)
			gi.cprintf(ent, PRINT_HIGH, "You must be at least level 8 to train this.\n");
...
void Kots_CharacterThrowOther(edict_t *ent)
{
	...
	else
	{
		...
		if (best)
		{
			...
			ent->character->next_throw = level.time + 3.0; //Throw Cooldown For all Throws.
