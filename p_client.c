...
void ClientBeginServerFrame(edict_t *ent)
{
	...
	//Idle stuff
	// idle frame counter
	if ((ent->velocity[0] != 0) || (ent->velocity[1] != 0) || (ent->velocity[2] != 0) || (ent->client->buttons & BUTTON_ATTACK))
	{
		ent->client->idle_frames = 0;
		//Check to see if the player is cloaked AND crouching
		if (ent->character->is_cloaked = true && ent->client->ps.pmove.pm_flags & PMF_DUCKED)
		{
			ent->client->idle_frames = 0;
		}
	}

	else if ((ent->health > 0) && (!ent->client->pers.spectator) && (ent->movetype != MOVETYPE_NONE))
	{
